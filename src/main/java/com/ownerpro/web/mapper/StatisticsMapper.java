...
public interface StatisticsMapper extends MyMapper<Article> {
    ...
    @Select("SELECT count(*) as value, name FROM article NATURAL JOIN article_writer NATURAL JOIN writer GROUP BY name")
    List<StatisticResponse> countByWriter();

    @Select("SELECT count(*) as value, name  FROM article NATURAL JOIN article_keyword NATURAL JOIN keyword  GROUP BY name")
    List<StatisticResponse> countByKeyword();

    @Select("SELECT count(*) as value, name  FROM article NATURAL JOIN article_type NATURAL JOIN type  GROUP BY name")
    List<StatisticResponse> countByType();

    @Select("SELECT count(*) as value, name  FROM article NATURAL JOIN article_area NATURAL JOIN area  GROUP BY name")
